  ...
  return ({ context, width, height, playhead }) => {
	...
	*/
	...
	for (var x = 0; x < width/2; x++) {
		if(x % 20 == 0) {
			context.moveTo(x,0);
			context.lineTo(width/2,width/2-x);
			context.stroke()
		}
	}
	...
	for (var x = 0; x < width/2; x++) {
		if(x % 20 == 0) {
			context.moveTo(width/2,x-10);
			context.lineTo(width,x-width/2);
			context.stroke()
		}
	}
	...
	for (var x = 0; x < width/2; x++) {
		if(x % 20 == 0) {
			//context.fillStyle = tinycolor('grey').lighten(100-x/width/2).toString();
			context.moveTo(width/2+x,width/2);
			context.lineTo(width/2+width+x,width/2-width);
			context.lineTo(width/2+width+x,width/2+width);
			context.lineTo(width/2+x,width/2);
			context.stroke()
		}
	}
